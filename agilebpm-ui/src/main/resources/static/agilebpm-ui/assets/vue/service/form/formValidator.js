...
function setValidity(validity,vnode){
	...
	if(validity === true){
		...
		vnode.elm.classList.remove("ab-invalid");
		vnode.elm.classList.add("ab-valid");
		
	}else{
		formContext.$validity[vnode.data.attrs.desc] = validity;
		
		vnode.elm.classList.add("ab-invalid");
		vnode.elm.classList.remove("ab-valid");
	}
}
...
FormValidator.format = function(msg,args){
	...
	if (!arr instanceof Array) //不是数组类型的
		args = [args];
	//数组类型的
